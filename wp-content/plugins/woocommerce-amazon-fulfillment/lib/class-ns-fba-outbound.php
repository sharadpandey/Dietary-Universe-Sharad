...
if ( ! class_exists( 'NS_FBA_Outbound' ) ) {
	...
	class NS_FBA_Outbound {
		...
		function create_fulfillment_address( $order_id ) {
			...
			$shipping_name = $shipping_name_raw = get_post_meta( $order_id, '_shipping_first_name', true ) . ' ' . get_post_meta( $order_id, '_shipping_last_name', true );
		...
		function send_fulfillment_order( $order_id, $is_manual_send = false ) {
			...
			// previously only caught FBAOutboundServiceMWS_Exception
			// expanded to Exception to handle additional cases
			} catch (Exception $ex) {
		...
		function create_log_summary_html ( $shipping, $speed, $address, $order ) {
			...
			$summary .= '<table><tbody><tr><th'.$style.'>Product</th><th'.$style.'>QTY</th><th'.$style.'>Price</th></tr>';
			
			if ( $this->ns_fba->wc->is_woo_version( '3.0' ) ) {
				$summary .= wc_get_email_order_items( $order );
			} else {		
				$summary .= $order->email_order_items_table( array( 'show_sku' => true ) ) . '</tbody></table><br />';
			}	
