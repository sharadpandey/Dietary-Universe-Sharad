	...
	class NS_FBA_WooCommerce {
		...
		function manage_new_product_columns( $columns ) {
			...
			$new_columns = array_slice( $columns, 0, 1, true ) +
		    	array( 'product_fba_status' => __( 'FBA', $this->ns_fba->text_domain ), ) +
		...
		/**
		 * Add a checkbox option to the Shipping tab of the product data
		 */
		...
		public function process_order_meta_box_actions( $order ) {
			if ( $this->ns_fba->is_debug ) {
				$message = 	"<h2>DEBUG on manual send to FBA order: #$order->get_order_number() </h2>" .
			...
			$this->ns_fba->outbound->send_fulfillment_order( $order->id, true );
